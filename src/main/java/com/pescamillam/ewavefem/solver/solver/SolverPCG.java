package com.pescamillam.ewavefem.solver.solver;
...
import com.pescamillam.ewavefem.solver.model.Dof;
...
public class SolverPCG extends Solver {
    ...
    private void setSparseRowStructure() {
        ...
        int lrow = fem.nDf *((fem.nDf == 2) ?
                                 FemConstants.maxRow2D : FemConstants.maxRow3D);
        ...
        for (int iel = 0; iel < fem.nEl; iel++) {
            for (int anInd : fem.elems[iel].ind) {
                ...
                for (int anInd1 : fem.elems[iel].ind) {
                    ...
                    if (k==prow[ii+1]) {
                        LOGGER.info("PCG sparse-row structure: not enough space for node " + ii);
                    }
                }
            }
        }
    ...
    private void displacementBC() {
        ...
        while (it.hasNext()) {
            ...
            for (int k = prow[j]; k < prow[j+1]; k++)
                if (coln[k] == j) A[k] = FemConstants.bigValue;
                else              A[k] = 0.0;
        }
    }
    ...
    public int pcg(double x[]) {
        ...
        double gammai = 1;
        int iter;
        for (iter = 0; iter < FemConstants.maxIterPcg; iter++) {
            //  w = (M-1)*r
            for (int i = 0; i < neq; i++)
                w[i] = md[i]*r[i];
            // gam = (r,w)
            ...
            if (err < FemConstants.epsPCG) return (iter + 1);
        }
        return (iter);
    }
