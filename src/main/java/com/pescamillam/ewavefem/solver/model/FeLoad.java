...
import com.pescamillam.ewavefem.solver.main.FemConstants;
...
public class FeLoad extends FeLoadData  {
    ...
    ListIterator <Dof> itnf;
    ListIterator <ElemFaceLoad> itsf;

    // Construct finite element load.
    // fem - finite element model
    public FeLoad(FeModel fem) {
        FeLoad.fem = fem;
        reader = FeModel.reader;
    ...
    public boolean readData( ) {
        ...
        return readDataFile(reader, true);
    }
    ...
    private boolean readDataFile(FeScanner es,
        ...
        if (newLoad) {
            scaleLoad = 0;
            nodForces = new LinkedList<Dof>();
            itnf = nodForces.listIterator(0);
            surForces = new LinkedList<ElemFaceLoad>();
    ...
    private void readNodalForces(FeScanner es) {
        ...
        if (idf == -1) {
            LOGGER.info("nodForce direction should be x/y/z. Specified:" + s);
        }

        if (!es.hasNextDouble()) {
            LOGGER.info("nodForce value is not a double: " + es.next());
        }
    ...
    private void readSurForces(FeScanner es) {
        ...
        if (dir == -1) {
            LOGGER.info("surForce direction should be x/y/z/n. Specified:"+s);
        }
        int iel = es.readInt();
    ...
    private void createBoxSurForces(FeScanner es) {
        ...
        int dir = Util.direction(s);
        if (dir == -1) {
            LOGGER.info("boxSurForce direction should be x/y/z/n. Specified:" + s);
        }

        if (!es.hasNextDouble()) {
            LOGGER.info("boxSurForce value is not a double: " + es.next());
        }
    ...
    public void assembleRHS() {
        ...
        while (itsf.hasNext()) {
            ...
            if (elm.equivFaceLoad(efl)==-1) {
                LOGGER.info("surForce does not match any face of element: " + efl.iel);
            }
        ...
        ListIterator<Dof> itdbc = fem.defDs.listIterator(0);
        while (itnf.hasNext()) {
            d = itdbc.next();
            RHS[d.dofNum-1] = FemConstants.bigValue * d.value;
        }
    }
